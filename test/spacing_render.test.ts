...
test("\\fsp increases line width", async () => {
  const plain = BASE.replace("%TEXT%", "{\\\\bord0\\\\shad0}Spacing");
  const spaced = BASE.replace("%TEXT%", "{\\\\bord0\\\\shad0\\\\fsp6}Spacing");

  const parsedPlain = parseASS(plain, { onError: "collect", strict: false, preserveOrder: true });
  const parsedSpaced = parseASS(spaced, { onError: "collect", strict: false, preserveOrder: true });
  expect(parsedPlain.ok).toBe(true);
  expect(parsedSpaced.ok).toBe(true);

  const base = await renderFrame(parsedPlain.document, 500, 320, 200);
  const spacedResult = await renderFrame(parsedSpaced.document, 500, 320, 200);

  const b0 = boundsFromLayers(base.layers);
  const b1 = boundsFromLayers(spacedResult.layers);
