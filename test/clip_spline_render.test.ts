...
test("vector spline \"s/p/c\" clip reduces coverage", async () => {
  const plain = BASE.replace("%TEXT%", "{\\\\bord0\\\\shad0\\\\pos(40,120)}MMMMMM");
  const clipped = BASE.replace(
    "%TEXT%",
    "{\\\\bord0\\\\shad0\\\\pos(40,120)\\\\clip(1,m 0 0 s 50 0 100 50 100 100 p 50 150 0 100 c)}MMMMMM"
  );
