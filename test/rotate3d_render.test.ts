...
Dialogue: 0,0:00:01.00,0:00:02.00,Default,,0,0,0,,{\\bord0\\shad0\\pos(160,100)\\org(160,100)\\frx45}Hello
`;
...
test("\\frx tilts glyphs in 3D", async () => {
  ...
  const tilted = await renderFrame(parsed.document, 1500, 320, 200);

  const b0 = boundsFromLayers(normal.layers);
  const b1 = boundsFromLayers(tilted.layers);
  const h0 = b0.maxY - b0.minY;
  const h1 = b1.maxY - b1.minY;

  expect(h1).toBeLessThan(h0 * 0.95);
});
