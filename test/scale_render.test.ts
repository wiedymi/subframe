...
Dialogue: 0,0:00:00.00,0:00:01.00,Default,,0,0,0,,{\\bord0\\shad0}Hello
Dialogue: 0,0:00:01.00,0:00:02.00,Default,,0,0,0,,{\\bord0\\shad0\\fscx200}Hello
Dialogue: 0,0:00:02.00,0:00:03.00,Default,,0,0,0,,{\\bord0\\shad0\\fscy200}Hello
`;
...
test("\\fscx scales width", async () => {
  ...
  const scaled = await renderFrame(parsed.document, 1500, 320, 200);

  const b0 = boundsFromLayers(normal.layers);
  const b1 = boundsFromLayers(scaled.layers);
...
test("\\fscy scales height", async () => {
  const parsed = parseASS(ASS, { onError: "collect", strict: false, preserveOrder: true });
  expect(parsed.ok).toBe(true);

  const normal = await renderFrame(parsed.document, 500, 320, 200);
  const scaled = await renderFrame(parsed.document, 2500, 320, 200);
