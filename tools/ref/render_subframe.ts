...
import { renderFrameFromDocument } from "../../src/core/render";
...
if (import.meta.main) {
  ...
  if (!ass || !out || !w || !h) {
    console.error("Usage: bun run tools/ref/render_subframe.ts --ass <file.ass> --time <ms> --w <width> --h <height> [--fonts <dir>] --out <out.png>");
    process.exit(1);
  }
  ...
  input.then(async (text) => {
    ...
    const rendered = await renderFrameFromDocument(parsed.document, timeMs, w, h);
    console.log(
      `subframe: events=${rendered.document.events.length} active=${rendered.events.length} time=${timeMs} layers=${rendered.result.layers.length}`
    );

    const png = new PNG({ width: w, height: h });
    png.data.fill(0);

    // Composite bitmap layers into RGBA frame.
    ...
    // Convert from premultiplied to straight alpha (match libass output).
    for (let y = 0; y < h; y++) {
      const row = y * w * 4;
      for (let x = 0; x < w; x++) {
        const idx = row + x * 4;
