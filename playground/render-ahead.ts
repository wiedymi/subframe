...
export interface RenderAheadDeps {
  ...
  render(doc: SubtitleDocument, timeMs: number, w: number, h: number): Promise<RenderResult>;
  ...
  present(frame: BufferedFrame): void;
  ...
  release?(frame: BufferedFrame): void;
...
export class RenderAheadPlayer {
  private deps: RenderAheadDeps;
  ...
  private buffer = new Map<number, BufferedFrame>();
  ...
  constructor(deps: RenderAheadDeps, opts: RenderAheadOptions = {}) {
    this.deps = deps;
  ...
  private releaseFrame(frame: BufferedFrame): void {
    try {
